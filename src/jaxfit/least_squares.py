...
class LeastSquares:
    ...
    def least_squares(
        ...
        if loss not in self.ls.IMPLEMENTED_LOSSES and not callable(loss):
            raise ValueError(
                "`loss` must be one of {0} or a callable.".format(
                    self.ls.IMPLEMENTED_LOSSES.keys()
                )
            )
        ...
        if callable(loss):
            ...
            if rho.shape != (3, m):
                raise ValueError(
                    "The return value of `loss` callable has wrong " "shape."
                )
        ...
        if jac is None and (max_nfev is None or max_nfev > 1):
            result.nfev += result.njev
